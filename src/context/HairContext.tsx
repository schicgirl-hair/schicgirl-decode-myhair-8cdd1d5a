...
export function HairProvider({ children }: { children: ReactNode }) {
  ...
  const generateResults = () => {
    const results = generateDiagnosis(state.answers);
  ...
  const reset = () =>
    setState({ answers: {}, currentStep: 0, results: null, email: "", isPaid: false });

  return (
    <HairContext.Provider value={{ ...state, setAnswer, setStep, generateResults, setEmail, setPaid, reset }}>
      {children}
